...
export async function installCondaMac() {
  ...
  const localInstallerPath = path.join(getInstallDirectory(), 'setup_miniconda.sh');
...
export async function installCondaWindows() {
  ...
  const localInstallerPath = path.join(getInstallDirectory(), 'setup_miniconda.exe');
...
export async function installToolsWithConda(installList) {
...
export async function verifyCondaPackage(packageName) {
  const condaEnvDir = getMiniCondaEnvironmentPath();
